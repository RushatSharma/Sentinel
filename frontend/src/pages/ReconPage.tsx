...
export default function ReconPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = client.subscribe(
      ...
      (response: any) => {
        ...
        if (isNewScan) {
          ...
          // CHANGED TO scan_mode
          if (
            doc.user_id === user.$id &&
            doc.scan_mode === "OSINT Recon" &&
            doc.target_url === domain
          ) {
            const reportData =
            ...
            setIsScanning(false);
            if (reportData.infrastructure?.length > 0) setActiveNodeIdx(0);
          }
        }
      },
    );
    ...
    const intervalId = setInterval(async () => {
      try {
        console.log("🔄 Polling Appwrite for new scan results...");
        ...
        if (res.documents.length > 0) {
          // CHANGED TO d.scan_mode
          const doc = res.documents.find(
            (d: any) =>
              d.user_id === user.$id &&
              d.scan_mode === "OSINT Recon" &&
              d.target_url === domain,
          );
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/recon`, {
        method: "POST",
      ...
      if (!response.ok) throw new Error("Failed to initiate OSINT scan.");
    } catch (err: any) {
      ...
      <div className="container relative z-10 mx-auto px-4 pt-8 max-w-[1600px] flex flex-col flex-grow">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-sentinel-blue/10 border border-sentinel-blue/20 mb-4">
            ...
            <span className="text-sm font-medium text-sentinel-blue uppercase tracking-wider">
          ...
          <p className="text-base md:text-lg text-muted-foreground mb-6 leading-relaxed max-w-5xl mx-auto">
            Reconnaissance is the critical first phase of the Cyber Kill Chain.
            Deploy passive OSINT (Open-Source Intelligence) to uncover hidden
            subdomains, shadow APIs, and forgotten staging servers. By querying
            global certificate transparency logs and threat intelligence
            databases (like AlienVault and HackerTarget), Sentinel maps a
            target's entire external digital footprint without ever sending a
            single packet to their servers—revealing the exact attack surface a
            real-world threat actor would exploit.
          </p>

          <form
            onSubmit={handleScan}
            className="w-full relative max-w-3xl mx-auto"
          >
            <motion.div
          ...
          >
            ...
            <div className="p-5 font-mono text-[11px] leading-relaxed text-muted-foreground overflow-y-auto flex-1 custom-scrollbar">
              ...
              {logs.map((log, i) => (
                <div
                  key={i}
                  className="animate-in fade-in slide-in-from-left-2 mb-2"
                >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center w-full">
                      ...
                      {isActive && (
                        <ArrowRight className="w-4 h-4 text-sentinel-blue animate-in slide-in-from-left-2" />
                      )}
                    </div>
                  </button>
                );
              ...
              {!activeNode ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground/50">
                  ...
                  <p className="font-mono text-sm">
                    Select a target to view intelligence profile.
                  </p>
                </div>
              ) : (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between border-b border-border pb-6">
                      <div>
                        ...
                        <p className="text-sm font-mono text-sentinel-blue">
                          {activeNode.subdomain}
                        </p>
                      </div>
                      <div
                        className={cn(
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-muted/30 border border-border p-5 rounded-lg">
                        <p className="text-sm text-muted-foreground leading-relaxed">
                          {intel.playbook ||
                            "Standard reconnaissance target. Attackers will map it, scan for open ports, and fingerprint the web server."}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-emerald-500/5 border border-emerald-500/20 p-5 rounded-lg">
                        <p className="text-sm text-muted-foreground leading-relaxed">
                          {intel.remediation ||
                            "Ensure software is patched and default credentials are removed."}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="pt-6 mt-6 border-t border-border">
                      <div className="flex items-start gap-4 bg-sentinel-blue/5 border border-sentinel-blue/20 p-5 rounded-lg">
                        ...
                        <div>
                          <span className="block text-xs font-bold text-sentinel-blue uppercase mb-2 tracking-wider">
                            Recommended Next Action
                          </span>
                          <span className="text-sm text-foreground/90">
                            {intel.next_step ||
                              "Run a Deep Scan to identify specific vulnerabilities."}
                          </span>
                        </div>
                      </div>
                    </div>
