...
export default function ApiFuzzerPage() {
  ...
  const [activeTab, setActiveTab] = useState<"terminal" | "findings">(
    "terminal",
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = client.subscribe(
      ...
      (response: any) => {
        ...
        if (isNewScan) {
          ...
          if (
            doc.user_id === user.$id &&
            doc.scan_mode === "API Fuzzer" &&
            doc.target_url === swaggerUrl
          ) {
            const reportData =
    ...
    const intervalId = setInterval(async () => {
      try {
        console.log("🔄 Polling Appwrite for API Fuzzer results...");
        ...
        if (res.documents.length > 0) {
          const doc = res.documents.find(
            (d: any) =>
              d.user_id === user.$id &&
              d.scan_mode === "API Fuzzer" &&
              d.target_url === swaggerUrl,
          );
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/fuzz-api`, {
        method: "POST",
      ...
      if (!response.ok) throw new Error("Failed to analyze API Blueprint.");

      // We do not set isScanning(false) here.
      // The Realtime listener will disable the scanning state once the background thread finishes.
    } catch (err: any) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const safeName = results.target_api
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase();
      ...
      <div className="container relative z-10 mx-auto px-4 pt-6 max-w-7xl flex flex-col gap-6 [perspective:1000px]">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 mb-6">
            ...
            <span className="text-[10px] font-bold text-purple-600 dark:text-purple-500 uppercase tracking-widest">
              Active API Exploitation
            </span>
          </div>

          <h1 className="text-4xl md:text-6xl lg:text-7xl font-display font-bold text-foreground mb-6 tracking-tight">
            API Blueprint{" "}
            <span className="text-purple-600 dark:text-purple-500">Fuzzer</span>
          </h1>

          <p className="text-base md:text-lg text-muted-foreground max-w-4xl mb-10 leading-relaxed">
            Modern applications run on APIs, making them the primary target for
            sophisticated threat actors. Provide a valid OpenAPI or Swagger JSON
            definition, and Sentinel’s intelligence engine will automatically
            ingest your routing schema. It dynamically maps all functional
            endpoints, reverse-engineers expected data structures, and launches
            high-intensity fuzzing attacks to hunt for critical OWASP API Top 10
            vulnerabilities—including Broken Object Level Authorization (BOLA),
            Unauthenticated Data Exposure, and Mass Assignment flaws.
          </p>
            ...
            >
              {[
                ...
                },
                {
                  ...
                  label: "Routes Mapped",
                  value:
                    results?.endpoints_mapped || (isScanning ? "..." : "0"),
                  large: true,
                },
                {
                  icon: Bug,
                  color:
                    results?.vulnerabilities?.length > 0 ? "red" : "emerald",
                ...
                <div className="p-3 overflow-y-auto custom-scrollbar flex-1 space-y-1">
                  ...
                  {results?.endpoint_list?.map((ep: any, idx: number) => (
                    ...
                    >
                      <span
                        className={cn(
                          "text-[9px] font-bold px-2 py-1 rounded border uppercase w-14 text-center shrink-0 mt-0.5",
                  ...
                  <div className="bg-muted/30 px-4 py-2 border-b border-purple-500/20 flex flex-wrap gap-2 justify-between items-center shrink-0">
                    <div className="flex gap-2">
                      <button
                        ...
                        className={cn(
                          "px-4 py-1.5 rounded-md text-xs font-mono font-bold transition-all flex items-center gap-2",
                          activeTab === "terminal"
                      ...
                      <button
                        ...
                        disabled={!results}
                        className={cn(
                          "px-4 py-1.5 rounded-md text-xs font-mono font-bold transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed",
                          activeTab === "findings"
                            ? "bg-purple-500/20 text-purple-600 dark:text-purple-400 border border-purple-500/30"
                            : "text-muted-foreground hover:text-foreground hover:bg-muted",
                        )}
                  ...
                  <div className="p-4 font-mono text-[11px] leading-relaxed text-muted-foreground overflow-y-auto flex-1 custom-scrollbar">
                    {logs.map((log, i) => (
                      <motion.div
                        ...
                        className="mb-1.5"
                      >
                        <span className="text-purple-600 dark:text-purple-500 mr-2">
                          ❯
                        </span>
                    ...
                    {isScanning && (
                      <div className="animate-pulse text-purple-600 dark:text-purple-500 mt-2">
                        _
                      </div>
                    )}
                    {results && (
                      <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        transition={{ duration: 1 }}
                        className="text-emerald-600 dark:text-emerald-400 mt-4 font-bold"
                      >
                        [✓] Fuzzing sequence completed successfully. Ready for
                        review.
                      </motion.div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-4">
                    {results.vulnerabilities.length === 0 ? (
                      <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        transition={{ duration: 1 }}
                        className="bg-card border border-emerald-500/30 rounded-xl p-10 flex flex-col items-center justify-center text-center shadow-sm"
                      >
                        ...
                        <p className="text-muted-foreground text-sm">
                          The targeted API schema withstands all fuzzing
                          payloads without exposing unauthorized data.
                        </p>
                      </motion.div>
                    ) : (
