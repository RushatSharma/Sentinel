...
export default function QuarantinePage() {
  ...
  useEffect(() => {
    if (!isScanning || !user || !artifact) return;
    ...
    const unsubscribe = client.subscribe(
      ...
      (response: any) => {
        ...
        if (isNewScan) {
          ...
          if (
            doc.user_id === user.$id &&
            doc.scan_mode === "Quarantine" &&
            doc.target_url === artifact
          ) {
            const reportData =
    ...
    const intervalId = setInterval(async () => {
      try {
        console.log("🔄 Polling Appwrite for Quarantine results...");
        ...
        if (res.documents.length > 0) {
          const doc = res.documents.find(
            (d: any) =>
              d.user_id === user.$id &&
              d.scan_mode === "Quarantine" &&
              d.target_url === artifact,
          );
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Containment analysis failed to initiate.");
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-background relative flex flex-col pb-12 font-sans overflow-x-hidden">
      ...
      <main className="container relative z-8 mx-auto px-4 pt-6 pb-6 max-w-7xl flex flex-col gap-8 [perspective:1200px] flex-grow">
        ...
        >
          <div
            className={cn(
              "inline-flex items-center gap-2 px-3 py-1 rounded-full border mb-6 transition-colors",
              theme.bgLight,
              theme.borderLight,
            )}
          >
          ...
          <p className="text-base md:text-lg text-muted-foreground max-w-4xl mb-10 leading-relaxed">
            Query global threat intelligence to detect malware, ransomware, and
            malicious infrastructure. Enter a file hash (MD5/SHA-256), a
            suspicious IP address, or a URL.
          </p>
          ...
          >
            <div
              className={cn(
                "absolute -inset-1 rounded-2xl blur opacity-50 group-hover:opacity-100 transition duration-1000 bg-gradient-to-r",
                theme.glow,
              )}
            />
            <div className="relative bg-card border border-border rounded-2xl shadow-xl overflow-hidden flex flex-col">
              <div className="bg-muted/30 border-b border-border p-3 flex justify-center gap-4">
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      "px-4 py-1.5 rounded-full text-xs font-bold font-mono transition-all flex items-center gap-2",
                      scanType === type.id
                        ? cn("text-white shadow-lg", theme.button)
              ...
              <div className="flex items-center p-2 h-16">
                ...
                <input
                  ...
                  placeholder={
                    scanType === "hash"
                      ? "Enter MD5, SHA-1, or SHA-256 hash (e.g., evil)..."
                      : scanType === "ip"
                        ? "Enter suspicious IPv4 address..."
                        : "Enter fully qualified domain or URL..."
                  }
                ...
                <Button
                  type="submit"
                  disabled={isScanning || !artifact.trim()}
                  className={cn(
                    "text-white font-bold tracking-wider text-sm h-10 px-8 transition-all rounded-xl mr-2",
                    theme.button,
                  )}
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          >
            <div
              className={cn(
                "bg-card border rounded-2xl shadow-xl overflow-hidden h-64 flex flex-col transition-colors",
                theme.borderLight,
              )}
            >
              <div
                className={cn(
                  "bg-muted/30 px-4 py-3 border-b flex items-center justify-between shrink-0",
                  theme.borderLight,
                )}
              >
                <span className="text-xs font-mono font-bold text-foreground/80 flex items-center gap-2">
                  <TerminalIcon className={cn("w-3.5 h-3.5", theme.text)} />{" "}
                  containment.log
                </span>
              </div>
              <div className="p-4 font-mono text-[10px] leading-relaxed text-muted-foreground overflow-y-auto flex-1 custom-scrollbar">
                {logs.length === 0 && !isScanning && !results && (
                ...
                {isScanning && (
                  <div className={cn("animate-pulse mt-1", theme.text)}>_</div>
                )}
              </div>
            </div>
            ...
            <AnimatePresence>
              {results && (
                <motion.div
                  ...
                  className={cn(
                    "bg-card border rounded-2xl p-6 shadow-sm text-center relative overflow-hidden transition-colors",
                    theme.borderLight,
                  )}
                >
                  <div
                    className={cn("absolute inset-0 opacity-10", theme.bgLight)}
                  />
                  <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-2 relative z-10">
                    Detection Ratio
                  </p>
                  ...
                  <p
                    className={cn(
                      "text-xs font-bold mt-4 uppercase tracking-widest relative z-10",
                      theme.text,
                    )}
                  >
                    {isMalicious
                      ? "MALICIOUS ARTIFACT DETECTED"
                      : "ARTIFACT IS SECURE & CLEAN"}
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <motion.div
            initial={{ opacity: 0, x: 30 }}
            animate={{ opacity: 1, x: 0 }}
            className="lg:col-span-8 bg-card border border-border rounded-2xl flex-1 overflow-hidden flex flex-col min-h-[500px] shadow-xl relative"
          >
            <div className="bg-muted/30 px-6 py-4 border-b border-border flex justify-between items-center z-10">
              <span className="text-sm font-bold flex items-center gap-2 uppercase tracking-tighter text-foreground">
                <Activity className={cn("w-4 h-4", theme.text)} /> Analysis
                Engines
              </span>
              {results && (
                <Button
                  ...
                  size="sm"
                  className={cn(
                    "h-7 text-xs border transition-colors hover:bg-muted",
                    theme.borderLight,
                    theme.text,
                    theme.bgLight,
                  )}
            ...
            <div className="flex-1 p-8 flex flex-col items-center justify-center bg-muted/20 relative overflow-y-auto custom-scrollbar">
              <div className="absolute inset-0 flex items-center justify-center opacity-5 pointer-events-none">
                {isMalicious ? (
                  <ShieldBan className={cn("w-64 h-64 text-orange-500")} />
                ) : (
                  <ShieldCheck className={cn("w-64 h-64 text-emerald-500")} />
                )}
              </div>
              ...
              {error && (
                <div className="p-4 text-orange-500 bg-orange-500/10 border border-orange-500/20 rounded-xl text-sm mb-4 z-10">
                  {error}
                </div>
              )}
                ...
                <div className="flex flex-col items-center w-full z-10">
                  {results?.threat_explanation && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className={cn(
                        "w-full max-w-3xl bg-card border rounded-xl p-5 mb-8 text-left shadow-lg",
                        theme.borderLight,
                      )}
                    >
                      <h3
                          ...
                          <motion.div
                            ...
                            animate={
                              isScanning
                                ...
                                : {
                                    backgroundColor: engine.isMalicious
                                      ? "rgba(249, 115, 22, 0.9)"
                                      : "rgba(16, 185, 129, 0.2)",
                                    rotateY: 180,
                                    scale: engine.isMalicious ? 1.1 : 1,
                                    boxShadow: engine.isMalicious
                            ...
                            transition={{
                              ...
                              delay: isScanning ? Math.random() * 2 : delay,
                            }}
                  ...
                  {isMalicious && results?.engine_results?.length > 0 && (
                    <motion.div
                      ...
                      className="w-full max-w-3xl bg-card border border-orange-500/30 rounded-xl p-4 mt-4 shadow-lg"
                    >
                      <h3 className="text-xs font-bold text-orange-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <ShieldAlert className="w-4 h-4" /> Threat Signatures
                        Detected
                      </h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                        {results.engine_results.map(
                          (engine: any, idx: number) => (
                            <div
                              key={idx}
                              className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-3 flex flex-col gap-1"
                            >
                              <span className="text-xs font-bold text-foreground/90">
                                {engine.engine}
                              </span>
                              <span className="text-[10px] font-mono text-orange-600 dark:text-orange-400 break-all">
                                {engine.result}
                              </span>
                            </div>
                          ),
