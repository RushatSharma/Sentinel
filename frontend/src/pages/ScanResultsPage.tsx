...
export default function ScanResultsPage() {
  ...
  // 1. Terminal Log Animation Effect (Sequential & Strict-Mode Safe)
  useEffect(() => {
    ...
    const deepLogs = [
      "[SYSTEM] Initializing Sentinel Deep Scan Engine...",
      `[NETWORK] Target locked: ${url}`,
      "[AUTH] Evaluating provided authentication configurations...",
      "[CRAWLER] Launching headless browser for dynamic DOM analysis...",
      "[HEURISTICS] Injecting XSS payloads into input vectors...",
      "[HEURISTICS] Fuzzing endpoints for SQL Injection vulnerabilities...",
      "[SCANNER] Analyzing security headers and CORS policies...",
      "[SCANNER] Searching for exposed sensitive files (.env, backups)...",
      "[INTEL] Cross-referencing findings with CVSS severity matrices...",
      "[SYSTEM] Compiling Deep Scan vulnerability report...",
    ];

    const quickLogs = [
      "[SYSTEM] Initializing Sentinel Heuristic Audit...",
      `[NETWORK] Target locked: ${url}`,
      "[SCANNER] Performing passive DOM analysis...",
      "[RECON] Extracting PII and exposed data from page source...",
      "[SCANNER] Evaluating SSL/TLS cryptographic health...",
      "[SCANNER] Checking for missing security headers (HSTS, CSP)...",
      "[SCANNER] Identifying exposed forms and unauthenticated endpoints...",
      "[INTEL] Aggregating compliance metrics (GDPR, PCI-DSS)...",
      "[SYSTEM] Compiling Quick Scan vulnerability report...",
    ];

    const logs = mode === "deep" ? deepLogs : quickLogs;

    // Async loop guarantees lines appear one-by-one without React batching them
    const runAnimation = async () => {
      for (const log of logs) {
        if (!isMounted) break;
        setScanLogs((prev) => [...prev, `> ${log}`]);
        // Random delay between 800ms and 1400ms for realistic typing effect
    ...
    return () => {
      isMounted = false; // Cleanup prevents memory leaks if component unmounts
    };
  }, [loading, url, mode]);

  // 1.5 API Trigger (Locked to prevent double-firing)
  useEffect(() => {
    if (!url) {
      navigate("/");
      return;
    ...
    if (processedUrlRef.current !== url) {
      ...
      const fetchScan = async () => {
        try {
          ...
          setError(
            "Connection Failed. Ensure the Sentinel Backend is running.",
          );
          setLoading(false);
        }
      };
  ...
  }, [url, navigate, mode, location.state, user]);

  // 2. Appwrite Realtime Listener + Polling Failsafe
  useEffect(() => {
    if (!loading || !user || !url) return;

    const databaseId =
      import.meta.env.VITE_APPWRITE_DATABASE_ID || "sentinel_db";
    const collectionId =
      import.meta.env.VITE_APPWRITE_COLLECTION_ID || "scan_history";
    const scanStartTime = Date.now() - 5000;

    const dbScanMode = mode === "deep" ? "Deep Scan" : "Quick Scan";

    // --- STRATEGY A: WebSockets (Instant) ---
    const unsubscribe = client.subscribe(
      `databases.${databaseId}.collections.${collectionId}.documents`,
      (response: any) => {
        console.log("🟢 Appwrite Realtime Event Received:", response);

        const isNewScan = response.events.some(
        ...
        if (isNewScan) {
          ...
          if (
            doc.user_id === user.$id &&
            doc.scan_mode === dbScanMode &&
            doc.target_url === url
          ) {
            const reportData =
              typeof doc.report_json === "string"
                ? JSON.parse(doc.report_json)
                : doc.report_json;
    ...
    // --- STRATEGY B: Polling Failsafe ---
    const intervalId = setInterval(async () => {
      try {
        console.log(`🔄 Polling Appwrite for ${dbScanMode} results...`);
        ...
        if (res.documents.length > 0) {
          ...
          if (doc) {
            ...
            if (docTime > scanStartTime) {
              console.log("✅ Scan found via Polling!");
              const reportData =
                typeof doc.report_json === "string"
                  ? JSON.parse(doc.report_json)
                  : doc.report_json;
  ...
  const getDoughnutData = () => {
    ...
    const medium = report.vulnerabilities.filter(
      (v: any) => v.severity === "Medium",
    ).length;
    const low = report.vulnerabilities.filter(
  ...
  const calculateCompliance = () => {
    ...
    const hasPII = vulns.some(
      (v: any) => v.type.includes("PII") || v.type.includes("Data"),
    );
    const hasInjection = vulns.some(
      (v: any) => v.type.includes("SQL") || v.type.includes("XSS"),
    );
    const hasAuthIssues = vulns.some(
      (v: any) =>
        v.type.includes("Auth") ||
        v.type.includes("Cookie") ||
    ...
    const pciStatus =
      hasInjection || hasAuthIssues || isCriticalFail ? "Review" : "Passing";
  ...
  const getNetworkStatus = () => {
    if (!report)
      return { port80: "Unknown", port443: "Unknown", ssh: "Unknown" };

    const networkVulns = report.vulnerabilities.filter(
      (v: any) => v.type === "Network Exposure",
    );

    const openPorts = networkVulns.map((v: any) =>
      String(v.url || v.details || ""),
    );

    const checkPort = (port: string) => {
      const isOpen = openPorts.some((p: string) => p.includes(port));
      if (isOpen)
        return { status: "OPEN", color: "text-red-500", bg: "bg-red-500/10" };
      return {
        status: "SECURE",
  ...
  const getRiskAssessment = (summary: {
    high: number;
    medium: number;
    low: number;
  }) => {
    if (summary.high > 0) {
      return {
        ...
        description:
          "This application contains severe security flaws that could lead to immediate data theft, financial loss, or total system compromise. It is unsafe for production deployment.",
      ...
      return {
        ...
        description:
          "Several security gaps were found. While not immediately catastrophic, these issues could be exploited by determined attackers to gain unauthorized access.",
      ...
      return {
        ...
        description:
          "The system is generally secure but contains minor configuration issues. These should be fixed during the next maintenance cycle to ensure best practices.",
        action: "MONITOR & FIX",
      };
    } else {
      return {
        ...
        title: "System Secure",
        description:
          "No significant vulnerabilities were detected. The application security posture is strong and adheres to current safety standards.",
  ...
  const calculateBusinessMetrics = (reportData: any) => {
    ...
    const financialRisk =
      reportData.financial_risk_total ||
    ...
    return {
      financialRisk: new Intl.NumberFormat("en-US", {
        style: "currency",
        currency: "USD",
        maximumFractionDigits: 0,
      }).format(financialRisk),
      devDays: devDays,
  ...
  const groupedVulnerabilities = useMemo(() => {
    ...
    report.vulnerabilities.forEach((vuln: any) => {
      const fix =
        vuln.remediation || vuln.fix || "No specific remediation provided.";
            ...
            <div className="p-6 h-[500px] overflow-y-auto font-mono text-sm relative custom-scrollbar">
              ...
              {scanLogs.map((log, i) => (
                <div key={i} className="mb-2 last:animate-pulse text-green-500">
                  <span className="text-sentinel-blue mr-3 opacity-70">
      ...
      <div className="container mx-auto px-4 pt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-0 mb-8 border rounded-xl bg-card/50 backdrop-blur-sm overflow-hidden divide-x divide-y md:divide-y-0">
          ...
          <div className="p-6 flex flex-col items-center justify-center text-center hover:bg-muted/50 transition-colors">
            ...
            <span className="text-4xl font-bold font-display text-sentinel-red">
              {report?.summary.high}
            </span>
          </div>
          <div className="p-6 flex flex-col items-center justify-center text-center hover:bg-muted/50 transition-colors">
          ...
          <div className="p-6 flex flex-col items-center justify-center text-center hover:bg-muted/50 transition-colors">
            ...
            <span
              className={cn(
                ...
                compliance.score < 70 ? "text-red-500" : "text-emerald-500",
              )}
            >
              {compliance.score}%
            </span>
        ...
        {report && (
          <div className="mb-8 space-y-6">
            <div
              className={cn(
                "border-2 rounded-xl p-6 shadow-sm relative overflow-hidden flex flex-col md:flex-row gap-6 items-center",
            ...
            >
              <div
                className={cn(
                  "w-24 h-24 rounded-full border-4 flex items-center justify-center bg-background shrink-0 shadow-lg",
                  getRiskAssessment(report.summary).color,
                  getRiskAssessment(report.summary).borderColor,
                )}
              >
                <span className="text-5xl font-display font-bold">
                  {getRiskAssessment(report.summary).grade}
                </span>
              </div>
              ...
              <div
                className={cn(
                  "px-4 py-2 rounded-lg border-2 font-bold uppercase tracking-wider text-sm whitespace-nowrap bg-background shadow-sm",
                  getRiskAssessment(report.summary).color,
                  getRiskAssessment(report.summary).borderColor,
                )}
              >
              ...
              <div className="bg-card border rounded-xl p-5 flex items-center gap-4 shadow-sm hover:border-sentinel-blue/50 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold font-display">
                    {calculateBusinessMetrics(report).financialRisk}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Real-time CVSS Valuation
                  </p>
                </div>
              </div>
              <div className="bg-card border rounded-xl p-5 flex items-center gap-4 shadow-sm hover:border-sentinel-blue/50 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold font-display">
                    {calculateBusinessMetrics(report).devDays} Days
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Estimated dev time to fix
                  </p>
                </div>
              </div>
              <div className="bg-card border rounded-xl p-5 flex items-center gap-4 shadow-sm hover:border-sentinel-blue/50 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold font-display">
                    {calculateBusinessMetrics(report).probability}%
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Based on severity vectors
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-card border rounded-xl p-6 shadow-sm">
              <h3 className="font-semibold mb-6 flex items-center gap-2">
                <Activity className="w-5 h-5 text-sentinel-blue" /> Threat
                Distribution
              </h3>
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm p-2 bg-muted/30 rounded">
                  <span className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-sentinel-red" />{" "}
                    Critical / High
                  </span>
                ...
                <div className="space-y-4 flex-1">
                  <div>
                    <div className="flex justify-between mb-1 text-xs font-medium">
                      <span>GDPR (EU)</span>
                      <span className={compliance.gdprColor}>
                        {compliance.gdpr}
                      </span>
                    </div>
                    <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden">
                      <div
                        className={cn("h-full", compliance.gdprBg)}
                        style={{ width: compliance.gdprWidth }}
                      />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between mb-1 text-xs font-medium">
                      <span>PCI-DSS</span>
                      <span className={compliance.pciColor}>
                        {compliance.pci}
                      </span>
                    </div>
                    <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden">
                      <div
                        className={cn("h-full", compliance.pciBg)}
                        style={{ width: compliance.pciWidth }}
                      />
                    </div>
                  </div>
                </div>
                          ...
                          >
                            ...
                            <TableCell className="font-medium">
                              <div className="flex items-center gap-2">
                                {vuln.type === "PII Exposure" && (
                                  <Fingerprint className="w-4 h-4 text-purple-500" />
                                )}
                                {vuln.type === "Network Exposure" && (
                            ...
                            <TableCell className="hidden md:table-cell">
                              {vuln.compliance &&
                              Object.keys(vuln.compliance).length > 0 ? (
                                <div className="flex gap-2">
                                  {Object.keys(vuln.compliance).map((k) => (
                                    <span
                                      key={k}
                                      className="text-[10px] border px-1.5 py-0.5 rounded bg-background uppercase text-muted-foreground"
                                    >
                                      {k}
                                    </span>
                                  ))}
                                      ...
                                      <div className="text-sm max-h-32 overflow-y-auto bg-background/50 p-2 rounded border">
                                        {vuln.groupedDetails.length > 1 ? (
                                          <ul className="list-disc pl-4 space-y-1">
                                            {vuln.groupedDetails.map(
                                              (d: any, i: number) => (
                                                <li key={i}>
                                                  {typeof d === "object"
                                                    ? d.url || JSON.stringify(d)
                                                    : d}
                                                </li>
                                              ),
                                            )}
                                          ...
                                          <p>
                                            {typeof vuln.groupedDetails[0] ===
                                            "object"
                                              ? vuln.groupedDetails[0].url ||
                                                JSON.stringify(
                                                  vuln.groupedDetails[0],
                                                )
                                              : vuln.groupedDetails[0]}
                                      ...
                                      <p className="text-sm leading-relaxed">
                                        {vuln.description ||
                                          (vuln.severity === "Critical"
                                            ? "Immediate exploitation possible. Data loss imminent. This vulnerability enables remote code execution or direct database access."
                                            : vuln.severity === "High"
                                              ? "Significant risk to integrity and availability. Attackers could manipulate data or deny service to legitimate users."
  ...
  return (
    <span
      className={cn(
        ...
        styles[severity] || "bg-gray-500/15 text-gray-600 border-gray-500/20",
      )}
    >
      {severity}
    </span>
